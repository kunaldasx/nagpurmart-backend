document.addEventListener("show.bs.modal", (event) => {
    if (event.target.id === "product-condition-modal") {
        ...
        const conditionId = triggerButton
            ? triggerButton.getAttribute("data-id")
            : null;
        let url = `${base_url}/${panel}/product-conditions/${conditionId}/edit`;

        const form = document.querySelector(
            "#product-condition-modal .form-submit",
        );
        const modalTitle = document.querySelector(
            "#product-condition-modal .modal-title",
        );
        const submitButton = document.querySelector(
            '#product-condition-modal button[type="submit"]',
        );
        const selectCategory = document.getElementById("select-category");
        let selectCategoryTom =
            selectCategory && selectCategory.tomselect
                ? selectCategory.tomselect
                : null;
        ...
        if (conditionId) {
            ...
            fetch(url, { method: "GET" })
                ...
                .then(async (responseData) => {
                    ...
                    form.querySelector('select[name="alignment"]').value =
                        data.alignment || "";

                    // Change form action to update route
                    form.setAttribute(
                        "action",
                        `${base_url}/${panel}/product-conditions/${conditionId}`,
                    );
            ...
            form.setAttribute(
                "action",
                `${base_url}/${panel}/product-conditions`,
            );
    ...
    if (event.target.id === "product-faq-modal") {
        ...
        const modalTitle = document.querySelector(
            "#product-faq-modal .modal-title",
        );
        const submitButton = document.querySelector(
            '#product-faq-modal button[type="submit"]',
        );
        const selectProduct = document.getElementById("select-product");
        let selectProductTom =
            selectProduct && selectProduct.tomselect
                ? selectProduct.tomselect
                : null;
        ...
        if (conditionId) {
            ...
            fetch(url, { method: "GET" })
                ...
                .then(async (responseData) => {
                    ...
                    form.setAttribute(
                        "action",
                        `${base_url}/${panel}/product-faqs/${conditionId}`,
                    );
...
document.addEventListener("click", function (event) {
    ...
    handleDelete(
        ...
        `/${panel}/product-conditions/`,
        "You are about to delete this Product Condition.",
    );
    handleDelete(
        event,
        ".delete-product",
    ...
    handleDelete(
        ...
        ".delete-product-faq",
        `/${panel}/product-faqs/`,
        "You are about to delete this Product Faq.",
    );
});
    ...
    if (!parentOption) {
        try {
            const res = await fetch(
                `${base_url}/${panel}/categories/search?find_id=${categoryId}`,
            );
...
async function loadProductAndSetValue(tomSelectInstance, productId) {
    ...
    if (!parentOption) {
        try {
            const res = await fetch(
                `${base_url}/${panel}/products/search?find_id=${productId}`,
            );
            const json = await res.json();
            // Assuming your endpoint returns an array of categories with id and name
            if (json && json.length) {
...
function toggleProductVariantSection() {
    ...
    if (value) {
        document
            .getElementById("variationsSection")
            .classList.toggle("d-none", !isVariant);
        document
            .getElementById("simpleProductSection")
            .classList.toggle("d-none", isVariant);
        // Show/hide the appropriate pricing containers
        document
            .getElementById("simplePricingContainer")
            .classList.toggle("d-none", isVariant);
        document
            .getElementById("variantPricingContainer")
            .classList.toggle("d-none", !isVariant);
...
document
    .getElementById("addRemovedVariantBtn")
    ?.addEventListener("click", () => showRemovedVariantsModal());
document
    .getElementById("removeAllVariantsBtn")
    ?.addEventListener("click", () => removeAllVariants());
    ...
    return (
        ...
        Object.entries(attrs)
            .map(([attrId, valueId]) => {
                ...
                let attrKey = Object.keys(dbAttributes).find(
                    (key) => dbAttributes[key].id === attrId,
                );
                ...
                return (
                    (attr?.name?.substring(0, 2).toUpperCase() || attrId) +
...
function showRemovedVariantsModal() {
    ...
    document.getElementById("removedVariantsList").innerHTML = removedVariants
        .map(
            (v) => `
    ...
    return axios
        ...
        .then((response) => {
            if (response.data.success) {
                ...
                if (
                    document.getElementById("productType").value === "variant"
                ) {
                    updateVariantPricing();
                } else {
                    initializeSimplePricing();
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text">${currencySymbol}</span>
                                                    <input type="number" class="form-control store-wholesale-price" name="store_pricing[${store.id}][wholesale_price]" step="0.01" min="0" value="${storeWholesalePrice}">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="input-group input-group-sm">
                                            ...
                                            .map((variant) => {
                                                ...
                                                if (
                                                    ...
                                                    if (
                                                        ...
                                                        const storePricing =
                                                            serverVariant.store_pricing.find(
                                                                (sp) =>
                                                                    sp.store_id ===
                                                                    store.id,
                                                            );
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="input-group input-group-sm">
                                                            <span class="input-group-text">${currencySymbol}</span>
                                                            <input type="number" class="form-control store-wholesale-price" name="variant_pricing[${store.id}][${variantId}][wholesale_price]" step="0.01" min="0" value="${storeWholesalePrice}">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="input-group input-group-sm">
...
function toggleVideoFields() {
    const selectedType =
        videoTypeSelect != null ? videoTypeSelect.value.toLowerCase() : "";
    if (
        videoLinkDiv != null &&
        videoUploadDiv != null &&
        videoLinkDiv != undefined &&
        videoUploadDiv != undefined
    ) {
...
$(document).ready(function () {
        ...
        if (catEl) {
            window.TomSelect &&
                new TomSelect(catEl, {
                    ...
                    searchField: "text",
                    placeholder:
                        typeof labels != "undefined" && labels.category
                            ? labels.category
                            : "Category",
    ...
    $(
        "#productVerificationStatusFilter, #productStatusFilter, #productTypeFilter, #productCategoryFilter, #productFilter",
    ).on("change", function () {
        table.ajax.reload(null, false);
    });
    ...
    (function () {
        ...
        const toggleReason = () => {
            const val =
                select != undefined && select != null && select != ""
                    ? select.value
                    : null;
...
$(document).ready(function () {
    document.addEventListener("click", function (event) {
        const updateProductStatus = event.target.closest(
    ...
    `;
        ...
        axios
            .post(`${base_url}/${panel}/products/${id}/update-status`)
            .then(function (response) {
                let data = response.data;
            ...
            .catch(function (error) {
                ...
                Toast.fire({
                    icon: "error",
                    title: "An error occurred while updating product status.",
                });
