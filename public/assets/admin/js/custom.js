            ...
            fetch(url, { method: "GET" })
                ...
                .then(async (responseData) => {
                    ...
                    if (typeof FilePond !== "undefined") {
                        ...
                        if (
                            data.active_icon !== null &&
                            data.active_icon !== undefined &&
                            activeIconUpload &&
                            data.active_icon !== ""
                        ) {
                            const activeIconPond =
                        ...
                        if (
                            ...
                            const backgroundImagePond = FilePond.find(
                                backgroundImageUpload,
                            );
                            if (backgroundImagePond) {
                                backgroundImagePond.addFile(
                                    data.background_image,
                                );
                            }
                        }
        ...
        if (conditionId) {
            ...
            fetch(url, { method: "GET" })
                ...
                .then(async (responseData) => {
                    ...
                    form.setAttribute(
                        "action",
                        `${base_url}/${panel}/faqs/${conditionId}`,
                    );
...
function toggleBackgroundFields(backgroundType) {
    const backgroundColorField = document.getElementById(
        "background-color-field",
    );
    const backgroundImageField = document.getElementById(
        "background-image-field",
    );
    const scrollBackgroundColorField = document.getElementById(
        "scroll-background-color-field",
    );
    const scrollBackgroundImageField = document.getElementById(
        "scroll-background-image-field",
    );
    const scrollBackgroundColorInput = document.querySelector(
        'input[name="scroll_background"][type="color"]',
    );
    const scrollBackgroundImageInput = document.querySelector(
        'input[name="scroll_background"][type="file"]',
    );

    if (backgroundType === "color") {
        backgroundColorField.style.display = "block";
        backgroundImageField.style.display = "none";
        if (scrollBackgroundColorField)
            scrollBackgroundColorField.style.display = "block";
        if (scrollBackgroundImageField)
            scrollBackgroundImageField.style.display = "none";
        if (scrollBackgroundColorInput)
            scrollBackgroundColorInput.disabled = false;
        if (scrollBackgroundImageInput)
            scrollBackgroundImageInput.disabled = true;
    } else if (backgroundType === "image") {
        backgroundColorField.style.display = "none";
        backgroundImageField.style.display = "block";
        if (scrollBackgroundColorField)
            scrollBackgroundColorField.style.display = "none";
        if (scrollBackgroundImageField)
            scrollBackgroundImageField.style.display = "block";
        if (scrollBackgroundColorInput)
            scrollBackgroundColorInput.disabled = true;
        if (scrollBackgroundImageInput)
            scrollBackgroundImageInput.disabled = false;
    } else {
        backgroundColorField.style.display = "none";
        backgroundImageField.style.display = "none";
        if (scrollBackgroundColorField)
...
document
    .querySelector(".generate-search-labels-button")
...
function addField(
    ...
    fieldDiv.className =
        "row mb-2 " + containerId.replace("Fields", "") + "-field";
    fieldDiv.innerHTML = `
        <div class="col-md-5">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("click", (e) => {
        if (
            e.target.classList.contains("remove-field") ||
    ...
    const googleLoginToggle = document.querySelector(
        'input[name="googleLogin"]',
    );
    ...
    const toggleCustomSmsFields = () => {
        if (customSmsFields) {
            customSmsFields.style.display = customSmsToggle.checked
                ? "block"
                : "none";
        }
    };
    ...
    const toggleFirebaseFields = () => {
        if (firebaseFields) {
            firebaseFields.style.display = firebaseToggle.checked
                ? "block"
                : "none";
        }
    };
    ...
    const validateMutuallyExclusiveSettings = () => {
        const customSmsEnabled = customSmsToggle
            ? customSmsToggle.checked
            : false;
        const firebaseEnabled = firebaseToggle ? firebaseToggle.checked : false;
        const googleLoginEnabled = googleLoginToggle
            ? googleLoginToggle.checked
            : false;
    ...
    if (customSmsToggle) {
        customSmsToggle.addEventListener("change", () => {
            console.log(
                "Custom SMS toggle changed, checked:",
                customSmsToggle.checked,
            );
    ...
    if (firebaseToggle) {
        firebaseToggle.addEventListener("change", () => {
            console.log(
                "Firebase toggle changed, checked:",
                firebaseToggle.checked,
            );
    ...
    if (googleLoginToggle) {
        googleLoginToggle.addEventListener(
            "change",
            validateMutuallyExclusiveSettings,
        );
    }
    ...
    if (appleLoginToggle) {
        appleLoginToggle.addEventListener(
            "change",
            validateMutuallyExclusiveSettings,
        );
    }
